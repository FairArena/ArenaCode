...
export async function hasActiveSubscription(customerExternalId: string): Promise<boolean> {
  try {
    const customerState = await polar.customers.getStateExternal({
      externalId: customerExternalId,
    });
    ...
    const targetProductId = getPolarProductId();
