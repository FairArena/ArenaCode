...
import { db } from "@arenacode/database/client";
import type { Prisma } from "@arenacode/database";
import { 
  getToolContracts, 
  modeSchema, 
  type ModeType, 
...
type ArenacodeUIMessage = UIMessage<ChatMessageMetadata, never, InferUITools<ToolContracts>>;

const submitSchema = z.object({
  id: z.string(),
  messages: z
    .array(
      z.custom<ArenacodeUIMessage>((value) => {
...
function hasPendingToolCalls(message: ArenacodeUIMessage) {
  ...
  .post(
    ...
    async (c) => {
      ...
      const previousMessages = Array.isArray(session.messages)
        ? (session.messages as unknown as ArenacodeUIMessage[])
        : [];
      const mergedMessages = [...previousMessages];
      
      for (const message of messages) {
        // Skip empty placeholder assistant messages from client
      ...
      const cleanedMessages = mergedMessages.filter(
        (m) => !(m.role === "assistant" && (!m.parts || m.parts.length === 0))
      );

      const nextMessages = await validateUIMessages<ArenacodeUIMessage>({
      ...
      return result.toUIMessageStreamResponse<ArenacodeUIMessage>({
