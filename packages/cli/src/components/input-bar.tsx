...
import { Mode } from "@arenacode/shared";
...
export function InputBar({ onSubmit, disabled = false }: Props) {
  ...
  useEffect(() => {
    try {
      const HISTORY_FILE = join(homedir(), ".arenacode", "history.json");
  ...
  }, [renderer, toast, dialog, navigate, mode, setMode, setModel, auth]);
  ...
  const handleSubmit = useCallback(() => {
    ...
    if (!isAuthenticated) {
      toast.show({ variant: "error", message: "Please sign in first by running /login" });
      return;
    }
    ...
    setHistory((prev) => {
      ...
      try {
        const HISTORY_FILE = join(homedir(), ".arenacode", "history.json");
        writeFileSync(HISTORY_FILE, JSON.stringify(next));
      } catch {}
      return next;
    });
    setHistoryIndex(-1);
    ...
    onSubmit(text);
    textarea.setText("");
  }, [disabled, onSubmit, isAuthenticated, toast, handleCommand])
  ...
  useKeyboard((key) => {
    ...
    if (key.name === "up") {
      ...
      setHistoryIndex((prevIndex) => {
        ...
        if (prevIndex === -1) {
          setDraftText(textarea.plainText);
          newIndex = history.length - 1;
        } else {
          newIndex = Math.max(0, prevIndex - 1);
        }

        textarea.setText(history[newIndex] || "");
        textarea.cursorOffset = (history[newIndex] || "").length;
        return newIndex;
      });
      ...
      setHistoryIndex((prevIndex) => {
        ...
        if (newIndex >= history.length) {
          textarea.setText(draftText);
          textarea.cursorOffset = draftText.length;
          return -1;
        } else {
          textarea.setText(history[newIndex] || "");
          textarea.cursorOffset = (history[newIndex] || "").length;
